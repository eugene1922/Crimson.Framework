	...
	public class GameStateSystem : ComponentSystem
	{
		...
		protected override void OnUpdate()
		{
			...
			Entities.With(_queryGameState).ForEach(
				...
				{
					Entities.WithAll<NeedDropItemTag>().ForEach(
						(Entity actorEntity, AbilityDropItem dropAbility) =>
						{
							var items = dropAbility.DropItems;
							for (var i = 0; i < items.Count; i++)
							{
								var item = items[i];
								var prefab = state.PrefabRepository.Get(item.ID);
								ActorSpawn.SimpleSpawn(prefab, dropAbility.DropPosition, dropAbility.DropRotation);
							}

							dropAbility.DropItems.Clear();
							EntityManager.RemoveComponent<NeedDropItemTag>(actorEntity);
						});
