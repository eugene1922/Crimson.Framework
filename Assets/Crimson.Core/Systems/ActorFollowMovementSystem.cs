		...
		protected override void OnUpdate()
		{
			Entities.With(_queryByInput).ForEach(
				(Entity entity, AbilityFollowMovement follow, ref ActorMovementData movement) =>
				{
					...
					if (math.abs(delta.x) < Constants.FOLLOW_MOVEMENT_AXIS_THRESH)
					{
						delta.x = 0f;
					}

					if (math.abs(delta.y) < Constants.FOLLOW_MOVEMENT_AXIS_THRESH)
					{
						delta.y = 0f;
					}

					if (math.abs(delta.z) < Constants.FOLLOW_MOVEMENT_AXIS_THRESH)
					{
						delta.z = 0f;
					}
			...
			Entities.With(_queryDirectly).ForEach(
					...
					ref ActorFollowMovementData data) =>
				{
					...
					if (follow.retainOffset)
					{
						movement.Position -= data.Origin;
					}
				}
			);
		}
