...
{
	public class PerkModifyWeaponsSpeed : MonoBehaviour, IActorAbilityTarget, IPerkAbilityForSpawned
	{
		...
		[ShowIf("spawnTargetEffect")] public GameObject targetEffectPrefab;
		...
		{
			var copy = target.GameObject.CopyComponent(this) as PerkModifyWeaponsSpeed;
		...
		{
			var p = target.CopyComponent(this) as PerkModifyWeaponsSpeed;
			if (p == null)
			{
				return;
			}
		...
		{
			var _dstManager = World.DefaultGameObjectInjectionWorld.EntityManager;
			...
			var targetLifespan =
				(AbilityLifespan)targetActor.Abilities.FirstOrDefault(ability => ability is AbilityLifespan);
