...
namespace Crimson.Core.Utils
{
	public static class TimerUtils
	{
		...
		{
			return obj.Timer == null ? null : (TimerAction?)obj.Timer.TimedActions.FirstOrDefault();
		}
		...
		public static TimerComponent GetOrCreateTimer(this GameObject obj, TimerComponent timer)
		{
			if (timer == null)
			{
				timer = obj.GetComponent<TimerComponent>();
				if (timer == null)
				{
					timer = obj.AddComponent<TimerComponent>();
				}
			}
			return timer;
		}
